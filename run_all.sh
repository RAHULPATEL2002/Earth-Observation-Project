#!/bin/bash
# End-to-end pipeline runner for Linux/macOS
# Captures all output to pipeline_run.log
...
python "$SCRIPT_DIR/train_model.py" --model cnn --epochs 10 --batch-size 32 --img-dir Data/rgb --lr 1e-3 --use-cuda --class-weight >> "$LOG_FILE" 2>&1
if [ $? -ne 0 ]; then
...
# Step 6: Grid map (optional, continue on error)
...
grep -E "(images scanned|labels created|Train:|Test:|Final Macro F1|Best model saved|PDF report saved)" "$LOG_FILE" || true
echo ""
